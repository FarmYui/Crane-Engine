...
namespace Crane
{
    namespace Utils
    {
        ...
        static void AttachColorTexture(uint32_t id, uint32_t samples, GLenum format, uint32_t width, uint32_t height, uint32_t index)
        {
            ...
            {
                glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, format == GL_RGBA8 ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, nullptr);
        ...
        if (m_ColorAttachmentSpecifications.size())
        {
            ...
            for (uint32_t i = 0; i < m_ColorAttachments.size(); i++)
            {
                ...
                case FramebufferTextureFormat::RGBA8:
                    Utils::AttachColorTexture(m_ColorAttachments.at(i), m_Specification.Samples, GL_RGBA8, m_Specification.Width, m_Specification.Height, i);
                    break;
                }
            }
        }
